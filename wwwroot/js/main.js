...
function uncheckmain(source, pclass) {
    alert(source);
    alert(pclass);
    var checkboxes = document.querySelectorAll('input[id="' + source.id + '" , class="' + pclass + '"]');

}
function checkallitemsbyname(source, parentid) {
    //alert(source);
    //alert(parentid);
    var checkboxes = document.querySelectorAll('input[name="' + parentid + '"]');
    for (var i = 0; i < checkboxes.length; i++) {
        if (checkboxes[i] != source)
            checkboxes[i].checked = source.checked;
    }
